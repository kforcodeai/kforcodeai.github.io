---
title: 'hermes-agent architecture, one component at a time'
description: 'A guided tour of hermes-agent architecture: the loop, the prompt, the cache plan, compaction, state, approvals and delegation, and what each choice costs.'
pubDate: '2026-08-31'
...
You might reasonably ask why a prompt needs tiers at all. It is entirely for the cache. Since a provider only reuses work when the bytes match from the beginning, you put the things that never change at the front and herd everything that might change into a tail you have already given up on.
...
Now the part that separates this from stamping markers on the last few messages. Three refusals:
...
Here is a question worth sitting with. The static prefix, the exact bytes of that stable tier, is not saved to disk. Only the full prompt is. So when a process restarts and picks up a stored session, how does it know where the cacheable part ends?
...
One mechanism here solves a problem you only hit at a certain scale, and I have not seen it in another harness.
...
Now the good bit. Six places in that loop *refund* an iteration, decrementing both counters together.

You might reasonably ask why you would ever hand a turn back. The rule is that the budget measures model turns the user asked for, not attempts the machinery made. So an iteration that never reached the provider does not count: a compression pass that fired before the request, a local runtime whose context turned out too small to hold the tool schema, a mid-turn correction that cancelled the in-flight call, a provider failover.
...
One small thing in the load path is worth stealing outright. The conversation load orders messages by their auto-incrementing row id, never by timestamp, and the comment explains why: `time.time()` is not monotonic. Sleep a laptop, step a clock with NTP, run under WSL, and a row written later can carry an earlier timestamp than the row before it. Sort by time and you can put an assistant's tool call *after* the tool's response, which is not a cosmetic problem. It is a malformed request the provider rejects.
...
### Two leases, and a hole worth knowing about
...
Here is the part worth teaching, and it took a production incident to arrive at. The cache is *asymmetric*. A success is cached normally. A failure arriving within a minute of the last success is thrown away, the tool stays available, and the next call re-probes.
...
Now the mechanism I most enjoyed reading. The model emits several tool calls at once. Do you run them in parallel or in order?
...
Here is a security detail worth stealing outright, and you can see the attack yourself if I show you the wrong version first. Imagine the bypass check read the environment on every call:
...
Here is a small design decision with an outsized payoff. Usage is normalized into four *disjoint* priced buckets, fresh input, output, cache reads and cache writes, and the familiar prompt-token total is computed from them rather than stored. Reasoning tokens ride along as a fifth count, but they are a breakdown of output rather than a peer of it, and nothing prices them.
...
## Three things I have not seen elsewhere

**It closes the loop on its own estimates.** Every harness guesses how large a request is. This one records the guess, pairs it with what the provider billed, corrects the next guess, and then asks afterwards whether the reduction it performed actually moved the number, and stops when the answer is no.

**The component that built a message declares where its cacheable part ends.** Not a delimiter search, not a heuristic: a registry the builder writes at construction time, because only the builder knows, and a wrong guess silently poisons the cache it was meant to help.

**A provider feature is treated as a written constraint with veto power.** "Prompt caching is sacred" is one of two lenses the guide says every change is reviewed against, and it holds in places that have nothing to do with caching: the interrupt path preserves byte-identical history, slash commands defer their effects to the next session, the two cheap context reducers ship disabled, and the persistence layer keeps a second copy of a message purely so the bytes replay identically.

## The tradeoffs, in one place

Those are the commitments this architecture makes. Here they are together, with what each one costs.

| Commitment | What it buys | What it costs |
|---|---|---|
| A byte-stable prefix, enforced in review | Most of the bill on a long conversation | Features that would mutate history are off, deferred, or gated |
| Reduction triggered and audited on billed usage | Fewer needless compactions, and a way to detect a floor no summary can fix | More durable state, and it degrades quietly where a provider reports no usage |
| Both cheap reducers off by default | The prefix survives inside a burst of turns | The junk stays in context, and the expensive reducer runs sooner |
| Approval-gated execution on the host, no sandbox | Full capability on your own machine | The boundary is a pattern list plus your attention |
| A child agent with no inherited history | The parent's context stays small | The child needs everything spelled out, and reports a self-report |
| Thousands of unit tests, no task-level benchmark | Mechanism is pinned precisely | Nothing detects a capability regression |

## What I would take, and what I would leave

Of everything above, the calibration loop is what I would take, without hesitation. Record your estimate before the call, pair it with the provider's reported usage when the response lands, fire your reduction on the corrected projection, then check the next real reading to see whether it helped. If you are still over, stop, your floor is the problem, and no schedule of summaries fixes a floor. It needs a number most provider APIs already return, and it degrades to the old behaviour where they do not.

The trap I would put on a wall is **decorate last**. Cache markers must be applied after every other transformation of the transcript. Marking a message rewrites its content from a plain string into a list of blocks, so a later pass that type-checks for a string skips it silently, and a tool result ending in a newline then ships unstripped while it sits in the marked window, and stripped once it rolls out. Same message, different bytes, consecutive turns, prefix broken exactly where you meant to protect it. You find out weeks later from a sag in your cache hit rate, with no error anywhere. This harness gets the order right and explains it in a comment. If you build one, make it an assertion.

What I would not take is the defaults. Remove the cache contract, a provider without a usable prefix cache, or an agent that is stateless per task, and those two zeros become the worst configuration on offer: no eviction, no amortization, one lossy summarization at the threshold. They are correct because of a constraint you may not share.

## Where this leaves us

So the rule I am keeping is narrower than the one I arrived with. Cheap context reduction never touches bytes the provider has already seen, and the next cheapest happens at a boundary you are already paying for: a compaction commit, or a cache expiry.

If you take one habit from this codebase, though, make it the comments. There is a genre of comment here that explains why the *obvious* fix is wrong and names the bug it caused, the non-atomic cache write that silently erased every learned context length, the reasoning-strip applied to the wrong list that corrupted stored conversations, the probe ladder that turned a user's million-token window into a quarter of it. Those record the shape of a failure, which is the thing a passing test cannot tell you.

And the question I would put to the next harness comes from the shape of this one. Its durable record is already append-only in the sense that matters: a compaction archives rows rather than deleting them, and they stay searchable under the same session id. What gets rewritten is the outgoing request, and that is what every gate, cooldown and durable counter in this post exists to protect. So why is the request assembled by editing history at all, rather than projected from it, so that what the model sees can change without changing a byte of what the provider has already seen?
