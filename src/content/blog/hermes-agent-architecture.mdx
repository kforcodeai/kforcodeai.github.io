...
A reading still over the line records a strike. Two strikes stop compaction, and the counters live on the session row rather than in memory, so restarting the process does not hand the loop a fresh start.
